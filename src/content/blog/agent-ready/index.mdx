...
<Aside>Since I wrote this, the spec moved my way: [MCP 2026-07-28](/mcp-goes-stateless/) made the stateless shape described here the core protocol. The `initialize` handshake and sessions are gone entirely, and the endpoint is now dual-era: it answers both old and new clients. If you're building this today, target 2026-07-28; the protocol surface for a read-only server is down to `server/discover`, `tools/list`, and `tools/call`.</Aside>

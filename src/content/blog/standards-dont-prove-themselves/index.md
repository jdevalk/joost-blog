...
excerpt: >-
  ...
  lookups. Their conclusion: not worth implementing. That conclusion reveals
  a misunderstanding of how web standards work, and a willingness to only
  pick sides after the debate is settled.
categories:
  - SEO
  - AI
...
toc: true
---
The SEO Framework [published data](https://github.com/sybrew/the-seo-framework/issues/732#issuecomment-4226935622) showing that across six months, 57 AI bots, and 180,000 AI-related requests to their site, not a single bot requested `llms.txt`. Their conclusion: implementing it would be "a waste of resources." Yoast SEO and Rank Math, they note, are wrong to support it.

The data is solid. I have no reason to doubt the methodology. And yet the conclusion is exactly backward.
...
Here's what makes this particularly relevant for SEO plugins. Yoast SEO runs on [over 10 million WordPress sites](https://wordpress.org/plugins/wordpress-seo/). Rank Math runs on millions more. When those plugins ship a feature, it doesn't create one data point. It creates millions of endpoints overnight.

That's the multiplier effect. SEO plugins don't just implement standards. They *create the supply* that gives crawlers a reason to consume them. When Yoast shipped XML sitemaps, it didn't just help individual sites get indexed. It made sitemaps ubiquitous enough that every search engine had to support them. When Yoast added IndexNow, it didn't just ping Bing for a few sites. It turned IndexNow from a niche protocol into infrastructure.
...
Which makes the framing of their tweet odd. They're not just saying "we're choosing not to implement this yet." They're saying Yoast and Rank Math are wrong to implement it. That's a strong claim to make from a position that has essentially no influence on the outcome they're measuring.

## The cost of waiting vs. the cost of trying

Implementing `llms.txt` in a WordPress plugin is trivial. You're generating a markdown file from data the plugin already has: titles, URLs, descriptions. The cost is measured in hours, not weeks. The ongoing maintenance cost is near zero.
...
**Content negotiation.** Cloudflare launched [Markdown for Agents](https://blog.cloudflare.com/markdown-for-agents/) in February 2026, serving markdown instead of HTML when an agent sends an `Accept: text/markdown` header. I [built the same thing independently](/markdown-alternate/) as a WordPress plugin, with richer metadata and dedicated `.md` URLs. Claude Code and other AI coding tools already send this header. The infrastructure is ready. The 80% token reduction over HTML is real and measurable.

**Schema endpoints and schema maps.** Instead of requiring agents to crawl every page to understand your site, you can serve your entire structured data graph through a single endpoint. I [built this into my Astro SEO library](/seo-graph/) and have it running on [this site](/schemamap.xml). Yoast launched the same concept as [Schema Aggregation](https://yoast.com/yoast-seo-march-3-2026/) in March 2026, in collaboration with Microsoft's NLWeb project. The approach is sound: give agents a complete, deduplicated map of your content in one request instead of making them reverse-engineer it from thousands of HTML pages. I'm proud that the company I co-founded is doing this, though I wish they'd moved faster.

**NLWeb.** Microsoft's open protocol for connecting websites with AI systems. It builds on schema.org, which LLMs already understand well. Still early, but the pieces are falling into place. A `<link rel="nlweb">` tag in your HTML head, a conversational endpoint, and your existing structured data. One line of HTML and you're discoverable.

**`llms.txt` itself.** A curated, markdown-formatted index of your most important content, placed at a well-known URL. Simple, low-cost, and easy to generate automatically.
...
I wrote about [optimizing crawling for the environment](/optimize-crawling-for-the-environment/) in 2022, showing how WordPress creates seven or more URLs for every single post, all of which get crawled, none of which add information. I followed that with a [proposal to invert the crawling model entirely](/optimize-crawling-lets-turn-things-around/): what if search engines only crawled URLs that sites explicitly listed in their sitemaps?

In 2025, I wrote about how [AI systems struggle to understand the modern web](/build-websites-like-2005/) because so much of it is buried behind JavaScript rendering and div soup. The irony: `llmstxt.org` was already pushing for exactly the kind of machine-readable content I was arguing for.

In 2026, I've been building the pieces. [Markdown Alternate](/markdown-alternate/) for serving markdown to agents. [seo-graph](/seo-graph/) for linked JSON-LD knowledge graphs. Schema endpoints and schema maps for agent discovery. NLWeb integration. Each piece is small. Together, they form a coherent architecture for machine-readable content.
...
What bothers me is the framing. They're not saying "we're watching this space." They're saying the plugins that *did* implement it are wrong. They're positioning "we only pick sides when the data is clear" as the principled stance.

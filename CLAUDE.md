...
- Joost excludes bulk SEO-title/description and image-alt maintenance from article modification dates. For future batches, add the Git commit trailer `Sitemap-Maintenance: seo-image-descriptions` (retain it in the squash commit when merging). This ignores only SEO frontmatter and image descriptions in that revision; article titles, excerpts, prose, code, image sources, and links still count. Do not skip an entire mixed commit or ignore individual editorial metadata corrections. Audited historical batches are recorded in `SEO_MAINTENANCE_COMMITS`.
